...
class EvoMap(gm.GameMap, Overview):
    ...
    def __call__(self, ctx: Context, poke: "Poke") -> bool:
        if not poke.player or poke.evolve_poke == "" or poke.lvl() < poke.evolve_lvl:
            return False
        ...
        poke.ico.remove()
        poke.ico.add(self, round(self.width / 2 - 4), round((self.height - 8) / 2))
        poke.moves.shine()
        self.outp.outp("Look!")
        ...
        for i in range(8):
            for j, k in zip([poke.ico, new.ico], [new.ico, poke.ico]):
                j.remove()
                k.add(self, round(self.width / 2 - 4), round((self.height - 8) / 2))
                time.sleep(SPEED_OF_TIME * 0.7 - i * 0.09999)
                loops.std(ctx)

        poke.ico.remove()
        new.ico.add(self, round(self.width / 2 - 4), round((self.height - 8) / 2))
        self.show()
        ...
        self.outp.outp(f"{poke.name} evolved into {new.name}!")
        time.sleep(SPEED_OF_TIME * 5)

        # Teach new moves if available
        for _ in range(max(len(asset_service.get_base_assets().pokes[new.identifier].attacks) - len(poke.attack_obs), 0)):
            LearnAttack(new)(ctx)

        # Update the player's Pokete list
        ctx.figure.pokes[ctx.figure.pokes.index(poke)] = new
        ...
        logging.info("[Poke] %s evolved into %s", poke.name, new.name)
        loops.std(ctx)
